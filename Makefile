...
compile: common.f90 disk.f90 auxiliary.f90 gas_state.f90 flux_feautrier.f90 input.in
	$(FC) $(FFLAGS) -c common.f90
	$(FC) $(FFLAGS) -c disk.f90
	$(FC) $(FFLAGS) -c auxiliary.f90
	$(FC) $(FFLAGS) -c gas_state.f90
	$(FC) $(FFLAGS) common.o auxiliary.o disk.o gas_state.o flux_feautrier.f90 -o flux_feautrier.x
clean:
	rm -f *.o *.mod
